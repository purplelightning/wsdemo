	...
	<view class="content">
		...
		@scroll="asideScroll" :scroll-top="tabScrollTop">
			<view v-for="item in slist" :key="item.id" class="s-list" :id="'main-'+item.id">
				...
				<view class="t-list">
					<view @click="navToList(item.id, titem.id)" v-if="titem.pid === item.id"
...
<script>
	export default {
		...
		methods: {
			...
			//右侧栏滚动，计算currentId，更新左侧tab
			asideScroll(e){
				if(!this.sizeCalcState){
					this.calcSize();
				}
				let scrollTop = e.detail.scrollTop;
				// 在左侧点击时有问题，需要加点距离，让左侧选中tab的与右侧列表第一个对准
				let tabs = this.slist.filter(item=>item.top <= scrollTop +20).reverse();
				if(tabs.length > 0){
